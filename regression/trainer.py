...
from regression.data.image import img_to_task, task_to_img
...
class RegressionTrainer:
    def __init__(self, exp, argv):
        ...
        args.exp_title = f'{exp}_maxpts-{args.max_num_pts}_minctx-{args.min_num_ctx}_mintar-{args.min_num_tar}'
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.args.num_epochs + 1):
            ...
            # On first epoch write param summary
            if epoch == self.start_epoch == 1:
                with torch.no_grad():
                    with Capturing() as output:
                        if self.args.model in ["np", "anp", "cnp", "canp", "bnp", "banp"]:
                            summary(self.model, batch, num_samples=self.args.train_num_samples)
                        else:
            ...
            for k, v in outs.items():
                self.tb.add_scalar(f'train/{k}', v.item(), epoch)
            self.tb.add_scalar('train/time', self.total_train_time, epoch)
    ...
    def eval(self, step=None):
        ...
        with torch.no_grad():
            for i, batch in tqdm(enumerate(eval_batches), ascii=True):
                ...
                if self.args.model in ["np", "anp", "bnp", "banp"]:
        ...
        line = f'{self.args.model}:{self.args.expid} {self.args.eval_kernel} '
        if self.args.t_noise is not None:
            line += f'tn {self.args.t_noise} '
        line += ravg.info()
        if step > -1:
            for k in ravg.keys():
                self.tb.add_scalar(f'val/{k}', ravg.sum[k] / ravg.cnt[k], step)
...
class GPTrainer(RegressionTrainer):
    ...
    def get_train_batch(self):
        return self.sampler.sample(
            ...
            max_num_pts=self.args.max_num_pts,
